...
- (void)addNewImg{
    if (_LQPhotoPicker_smallImageArray.count == _LQPhotoPicker_imgMaxCount) {
        ...
        UIAlertController *alertCtl = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"选择图片数量已达上线" preferredStyle:UIAlertControllerStyleAlert];
        
        UIAlertAction *actionAlert = [UIAlertAction actionWithTitle:@"晓得嘞" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
        }];
        [alertCtl addAction:actionAlert];
...
//-(NSData *)getBigImageDataWithPhAsset:(PHAsset *)asset
//{
//    
//    return [NSData data];
//}
