...
<UITableViewDataSource,UITableViewDelegate,TableActionCellDelegate,DiscoverHeaderViewDelegate,UIScrollViewDelegate,LQImgPickerActionSheetDelegate,SelectPhotoDelegate,AwardViewDelegate>
...
//@property(nonatomic,strong) UIView *editVc;

@end

const CGFloat kRefreshBoundary = 140.0f;
...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationBar.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.5];
...
#pragma mark --删除该动态
-(void)onDeleteInCell:(TableViewCell *)cell
{
//    [self loginAction];
    [self alertViewMessage:@"您确定要删除这条动态吗？"];
...
-(void)okDeleteClick
{
    ...
    [LWAlertView shoWithMessage:@"确认删除"];
...
//查看打赏情况
-(void) showAwardWithCell:(TableViewCell *)cell
{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    [self.headerView loadingViewAnimateWithScrollViewContentOffset:offset];
...
- (void)refreshBegin {
    [UIView animateWithDuration:0.2f animations:^{
        ...
        [self.headerView refreshingAnimateBegin];
...
- (void)refreshComplete {
    [self.headerView refreshingAnimateStop];
...
- (void)setup {
    ...
    [button setImage:[UIImage imageNamed:@"camera_icon_pyq"] forState:UIControlStateNormal];
    
    [button addTarget:self action:@selector(sendLifeView) forControlEvents:UIControlEventTouchUpInside];
    
    [button setImageEdgeInsets:UIEdgeInsetsMake(5, 20, 5, -5)];
    
    button.frame = CGRectMake(SCREEN_WIDTH-50, 0, 40, 40);
    
    button.backgroundColor = clearC;
    
    UIBarButtonItem *itemBar = [[UIBarButtonItem alloc]initWithCustomView:button];
...
{
    if (!_headerView) {
        _headerView = [[DiscoverHeaderView alloc]initWithFrame:CGRectMake(0, -64, SCREEN_WIDTH, 275.0f)];
...
#pragma mark ------discoverHeaderViewDelegate
//
-(void)sendLifeView
{
    [self loginAction];
    ...
    if (self.loginOrNot) {
        SendFriendsLifeController *sendVc = [[SendFriendsLifeController alloc]init];
        
        MainNavigationController *nav = [[MainNavigationController alloc]initWithRootViewController:sendVc];
        [self.navigationController presentViewController:nav animated:YES completion:nil];
    }
    
}
//headerViewDelegate Action
//更换背景
-(void)changeBackViewImage
{
//    LQImgPickerActionSheet *sheet = [[LQImgPickerActionSheet alloc]init];
//    
//    sheet.delegate = self;
//    
//    [sheet showImgPickerActionSheetInView:self];

    self.ccpView = [[CCPActionSheetView alloc]initWithActionSheetArray:@[@"更换相册封面",@"取消"]];
    [self.ccpView cellDidSelectBlock:^(NSString *indexString, NSInteger index) {
       
        HWLog(@"%@",indexString);
        
        if (index == 0) {
            
            SelectPhotoViewController *vc = [SelectPhotoViewController new];
            vc.delegate = self;
            WeakSelf(weakSelf);
...
{
    self.headerView.bgImageVc.image = image;
}
...
{
    [LWAlertView shoWithMessage:@"点击头像"];
}
