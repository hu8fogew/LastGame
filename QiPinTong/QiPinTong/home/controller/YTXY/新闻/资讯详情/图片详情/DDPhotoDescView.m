...
- (instancetype)initWithTitle:(NSString *)title desc:(NSString *)desc index:(NSInteger)index totalCount:(NSInteger)totalCount
{
	...
	if (self) {
		// 描述文本
		UITextView *textView = [[UITextView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 0)];
		textView.text = desc;
		textView.backgroundColor = [UIColor clearColor];
		textView.textColor = [UIColor colorWithWhite:0.896 alpha:1.000];
		textView.font = [UIFont systemFontOfSize:16];
		// 这个37算的我莫名其妙，得不到正确的textView高度！！！！
		textView.frame = CGRectMake(0, 0, SCREEN_WIDTH, textView.contentSize.height + 37);
		textView.userInteractionEnabled = NO;
		
		// self
		self = [[DDPhotoDescView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT - DescViewDefaultHeight, SCREEN_WIDTH, 999)];
		...
		indexLabel.x = SCREEN_WIDTH - indexLabel.width - 5;
...
- (void)swipe:(UISwipeGestureRecognizer *)recognizer
{
	if (recognizer.direction == UISwipeGestureRecognizerDirectionUp) {
		[UIView animateWithDuration:0.3 animations:^{
			self.y = SCREEN_HEIGHT - self.tag - 50;
		}];
	} else if (recognizer.direction == UISwipeGestureRecognizerDirectionDown) {
		[UIView animateWithDuration:0.3 animations:^{
			self.y = SCREEN_HEIGHT - DescViewDefaultHeight;
		}];
	} else {
		NSLog(@"wocao");
	}
