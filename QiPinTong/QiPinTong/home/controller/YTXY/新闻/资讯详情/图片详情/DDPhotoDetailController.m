...
- (JT3DScrollView *)imageScrollView
{
	if (_imageScrollView == nil) {
		...
		_imageScrollView = [[JT3DScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH - 40)];
		_imageScrollView.contentSize = CGSizeMake(_photoModel.photos.count * SCREEN_WIDTH, SCREEN_WIDTH - 40);
		...
		for (int i = 0; i < self.photoModel.photos.count; i++) {
			...
			DDPhotoScrollView *photoScrollView = [[DDPhotoScrollView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH * i, 0, SCREEN_WIDTH, SCREEN_WIDTH - 40) urlString:detailModel.imgurl];
...
- (DDBottomView *)bottomView
{
	if (_bottomView == nil) {
		...
		_bottomView.frame = CGRectMake(0, SCREEN_HEIGHT - 40, SCREEN_WIDTH, 40);
