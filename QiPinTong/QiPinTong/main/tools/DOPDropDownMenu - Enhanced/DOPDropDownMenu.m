...
typedef NS_OPTIONS(NSUInteger, CollectionSelectState) {//collectionCell的选中和未选中的状态
    CollectionSelectStateNormal       = 0,
    CollectionSelectStateSelected     = 1,                  // flag usable by app (see below)
};

@interface QPTCollectionViewCell : UICollectionViewCell;

@property(nonatomic,strong)UILabel *textLabel;

@property (nonatomic ,assign) CollectionSelectState selectState;
//-(void)removeAccessoryView;

@end


@implementation QPTCollectionViewCell
#define SelectColor [UIColor colorWithRed:246/255.0 green:79/255.0 blue:0/255.0 alpha:1]
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.layer.borderColor = [HWColor(200, 200, 200) CGColor];
        self.layer.masksToBounds = YES;
        self.backgroundColor = whiteC;
        
        //设置label
        _textLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        _textLabel.backgroundColor = clearC;
        _textLabel.font = [UIFont systemFontOfSize:subTitleSize];
        _textLabel.textColor = deepGrayColor;
        _textLabel.textAlignment = NSTextAlignmentCenter;
        [self addSubview:_textLabel];
        
        self.selectState = CollectionSelectStateNormal;
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _sectionLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, 0, frame.size.width-10, frame.size.height)];
        
        _sectionLabel.font = [UIFont systemFontOfSize:MTitleSize];
        _sectionLabel.textColor = SecondTitleColor;
...
@property (nonatomic ,copy) NSMutableArray *selectSiftArray;
//@property (nonatomic ,copy) NSMutableArray *allSelectTitleArray;
////
//@property (nonatomic, copy)NSMutableArray *allSelectImgArray;
...
- (UIColor *)indicatorColor {
    if (!_indicatorColor) {
        _indicatorColor = SecondTitleColor;
    }
...
- (UIColor *)separatorColor {
    if (!_separatorColor) {
        _separatorColor = SecondTitleColor;
    }
    return _separatorColor;
}

-(NSMutableArray *)selectSiftArray
{
    if (!_selectSiftArray) {
        _selectSiftArray = [NSMutableArray array];
        
    }
    return _selectSiftArray;
}
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:grayC andPosition:bgLayerPosition];
        [self.layer addSublayer:bgLayer];
        
        CALayer *lineLayer = [self createBgLayerWithColor:[UIColor whiteColor] andPosition:CGPointMake(bgLayerPosition.x, bgLayerPosition.y-1)];
        ...
        if (!self.isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
            ...
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:i row:0 item:(self.currentItem == 0 ? 0:self.currentItem)]];
            
#pragma mark -----没有二级
        }else {
            
            if (i<_numOfMenu-1) {
                titleString = [_dataSource menu:self titleForRowAtIndexPath:
                               [DOPIndexPath indexPathWithCol:i row:self.currentRow == 0 ? 0 : self.currentRow]];
            }else
            {//第四列名称保持不变
                titleString = @"刷新";
            }
        ...
        if (self.indicatorIsImageView) {
            ...
            CAShapeLayer *indicator = [self createIndicatorWithColor:self.indicatorColor andPosition:CGPointMake(title.frame.origin.x+title.frame.size.width+10, self.frame.size.height / 2+2)];
...
-(void)setShow:(BOOL)show{
    _show = show;
    if (!show) {
        if (self.currentIndexPath!=nil) {//回收之后回调
...
- (instancetype)initWithOrigin:(CGPoint)origin width:(CGFloat)width andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        _collectionVc = [[UICollectionView alloc]initWithFrame:CGRectMake(origin.x, self.botoom, SCREEN_WIDTH, 0) collectionViewLayout:flowLayout];
        _collectionVc.delegate = self;
        _collectionVc.dataSource = self;
        _collectionVc.backgroundColor = HWColor(249, 249, 249);
        [_collectionVc registerClass:[QPTCollectionViewCell class] forCellWithReuseIdentifier:@"CollectionCell"];
        [_collectionVc registerClass:[CollectionHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:@"headerView"];
        self.autoresizesSubviews = NO;
        _leftTableView.autoresizesSubviews = NO;
        _rightTableView.autoresizesSubviews = NO;
        _collectionVc.autoresizesSubviews = NO;
        
        
        
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(origin.x, self.botoom, dropDownViewSize.width, kButtomImageViewHeight)];
        _buttomImageView.image = [UIImage imageNamed:@"icon_chose_bottom"];
        
        
        _bottomSureView = [[UIView alloc]initWithFrame:CGRectMake(origin.x, self.botoom, SCREEN_WIDTH, bottomSureHeight)];
        _bottomSureView.layer.borderWidth = 1.0f;
        _bottomSureView.layer.borderColor = [grayC CGColor];
        _bottomSureView.layer.masksToBounds = YES;
        ...
        for (int i = 0; i<2; i++) {
            ...
            [btn setTitleColor:i == 0 ? kTextSelectColor : whiteC forState:UIControlStateNormal];
            btn.titleLabel.font = [UIFont systemFontOfSize:MTitleSize];
            
            [btn setBackgroundColor:i == 0 ? whiteC : kTextSelectColor];
            
            btn.frame = CGRectMake(0+i*SCREEN_WIDTH/2, 0, SCREEN_WIDTH/2, bottomSureHeight);
            
            [_bottomSureView addSubview:btn];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    if (displayByCollectionView) {
        UICollectionView *collectionView = _collectionVc;
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            [_collectionVc reloadData];
            
            if (_currentSelectedMenudIndex != -1) {
                //需要隐藏tableView
                [self animateTableView:_leftTableView show:NO complete:^{
                    
                    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView collectionView:collectionView title:_titles[tapIndex] forward:YES complecte:^{
                        self.show = YES;
                    }];
                }];
        ...
        if (tapIndex == _currentSelectedMenudIndex && _show) {
            ...
            if (_currentSelectedMenudIndex != -1) {
                // 需要隐藏collectionview
                [self animationCollectionView:_collectionVc show:NO complete:^{
...
- (void)backgroundTapped:(UITapGestureRecognizer *)paramSender
{
    ...
    if (displayByCollectionView) {
        ...
        [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView collectionView:_collectionVc title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
            _show = NO;
        }];
...
-(void)clickSureBtn:(UIButton *)sender
{
    if (sender.tag == 0) {
        ...
        [self.selectSiftArray removeAllObjects];
...
//#warning mark ----添加点击代理
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(didClickedMenu)]) {
            
            [_delegate didClickedMenu];
        }
...
/**collectionView向下弹出动画*/
-(void)animationCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)())complete
{
    if (show) {
        CGFloat collectioinViewHeight = 0;
        if (collectionView) {
            collectionView.frame = CGRectMake(_origin.x, self.botoom, SCREEN_WIDTH, 0);
            
            [self.superview addSubview:collectionView];
            collectioinViewHeight = SCREEN_HEIGHT-SCREEN_WIDTH*0.1-64-bottomSureHeight+2;
            
        }
        
        _bottomSureView.frame = CGRectMake(self.x, self.botoom, SCREEN_WIDTH, bottomSureHeight);
        [self.superview addSubview:_bottomSureView];
        
        
        [UIView animateWithDuration:0.2 animations:^{
            if (collectionView) {
                collectionView.frame = CGRectMake(_origin.x, self.botoom, SCREEN_WIDTH, collectioinViewHeight);
                
            }
            _bottomSureView.frame = CGRectMake(self.x, CGRectGetMaxY(_collectionVc.frame)-2, SCREEN_WIDTH, bottomSureHeight);
    ...
    }else
    {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [_bottomSureView removeFromSuperview];
        }];
    }
...
- (void)animateIdicator:(CAShapeLayer *)indicator background:(UIView *)background collectionView:(UICollectionView *)collectionView title:(CATextLayer *)title forward:(BOOL)forward complecte:(void(^)())complete{
    ...
    [self animateIndicator:indicator Forward:forward complete:^{
        [self animateTitle:title show:forward complete:^{
            [self animateBackGroundView:background show:forward complete:^{
                [self animationCollectionView:collectionView show:forward complete:^{
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row column:_currentSelectedMenudIndex]> 0) {
        ...
        indicator.position = CGPointMake(title.frame.origin.x+title.frame.size.width+10, indicator.position.y);
        return YES;
    }
    
    
}
- (void)confiMenuWithSelectItem:(NSInteger)item {//将第二个表的值传给menu
...
//        NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
//        HWLog(@"===%zd",currentSelectedMenudRow);
        return [self.dataSource menu:self
              numberOfItemsInRow:0 column:_currentSelectedMenudIndex];
    }else
    {
        HWLog(@"required method of dataSource protocol should be implemented");
        return 0;
    }
}
...
{
    QPTCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"CollectionCell" forIndexPath:indexPath];
    
//    if (!cell) {
//        cell = [[QPTCollectionViewCell alloc]initWithFrame:CGRectZero];
//    }
    
    if ([self.dataSource respondsToSelector:@selector(menu:titleForRowAtIndexPath:)]) {
        cell.textLabel.text = [self.dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        
        BOOL isbool = [self.selectSiftArray containsObject:cell.textLabel.text];
        
        if (isbool) {
            cell.selectState = CollectionSelectStateSelected;
            cell.textLabel.textColor = kTextSelectColor;
            cell.layer.borderColor = kTextSelectColor.CGColor;
            cell.layer.borderWidth = 1.0f;
        }else
        {
            cell.selectState = CollectionSelectStateNormal;
            cell.textLabel.textColor = deepGrayColor;
            cell.layer.borderColor = HWColor(200, 200, 200).CGColor;
            cell.layer.borderWidth = 1.0f;
        }
        
    }else
    {
        HWLog(@"dataSource method needs to be implemented");
    }
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        resuseIdentifier = @"headerView";
    }
    
    CollectionHeaderView *view = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:resuseIdentifier forIndexPath:indexPath];
    
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        view.sectionLabel.text = @"服务";
    }
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.delegate || [self.delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
        ...
        QPTCollectionViewCell *cell = (QPTCollectionViewCell *)[_collectionVc cellForItemAtIndexPath:indexPath];
        NSString *str = cell.textLabel.text;
//        HWLog(@"-=-=-=-=-=%@",str);
        [self.selectSiftArray addObject:str];
        cell.selectState = !cell.selectState;
        
        if (cell.selectState == 1) {
            cell.textLabel.textColor = kTextSelectColor;
            cell.layer.borderColor = kTextSelectColor.CGColor;
            cell.layer.borderWidth = 1.0f;
            ...
            cell.layer.borderColor = HWColor(200, 200, 200).CGColor;
            cell.layer.borderWidth = 1.0f;
            [self.selectSiftArray removeObject:str];
            
        }
        
        
//        HWLog(@"%@",self.selectSiftArray);
        
        if (_delegate && [_delegate respondsToSelector:@selector(menu:didSelectSiftArray:)]) {
            
            [_delegate menu:self didSelectSiftArray:self.selectSiftArray];
        }
...
{
//    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
//    title.string = [self.dataSource menu:self titleForRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:row]];
...
{
    return CGSizeMake((SCREEN_WIDTH-75)/4, 30);
}
