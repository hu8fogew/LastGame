...
-(UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        _collectionView = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, (hostCities.count/3==0?hostCities.count/3:hostCities.count/3+1)*40 + 40 + 60 + 60) collectionViewLayout:flowLayout];
...
- (void)viewDidLoad {
    ...
    //设置索引条目字体颜色
    self.mainTable.sectionIndexBackgroundColor = clearC;
    self.mainTable.sectionIndexColor = MainColor;
...
-(void)setupNavView
{
    ...
    [btn addTarget:self action:@selector(returnVC) forControlEvents:UIControlEventTouchUpInside];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (!_isSearch) {
        if (indexPath.section == 0) {
            return (hostCities.count/3==0?hostCities.count/3:hostCities.count/3+1)*40 + 40 + 60 + 60;
        }else{
            return 44;
        }
    }else{
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        self.selectCity(@"全国");
    }else if (indexPath.section == 1 && !isLocationFail){
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]){
        if (indexPath.section == 0) {
            view.sectionTitleLabel.text = @"默认";
        }else if (indexPath.section == 1) {
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        return CGSizeMake((self.collectionView.width-20)/3 - 20, 30);
    }else if (indexPath.section == 1){
        return CGSizeMake(self.collectionView.width/3, 30);
    }else{
        return CGSizeMake((self.collectionView.width-20)/3 - 20, 30);
    }
...
- (UIEdgeInsets)collectionView:(UICollectionView *)collectionView
    ...
    return UIEdgeInsetsMake(5, 10, 5, 30);
}
