...
#import "InvestorBackTableViewCell.h"

@interface InvestorBackTableViewCell()<LWAsyncDisplayViewDelegate>
...
@implementation InvestorBackTableViewCell
...
-(void)layoutSubviews{
    self.asyncView.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.investorBackLayout.investorCellHeight);
...
-(void)_layoutSubviews{
    ...
    self.editBtn.frame = self.investorBackLayout.iEditBtn;
    self.deleteBtn.frame = self.investorBackLayout.iDeleteBtn;
}
...
-(InvestorBackCellLayout *)investorBackLayout{
    if (!_investorBackLayout) {
        _investorBackLayout = [[InvestorBackCellLayout alloc]initWithInvestorBackCell];
        self.asyncView.layout = self.investorBackLayout;
    }
    return _investorBackLayout;
}
...
-(void)clickEditBtn:(UIButton *)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(investorCellEditBtnClick:)]) {
        [_delegate investorCellEditBtnClick:self ];
    }
}

-(void)clickDeleteBtn:(UIButton *)sender{
    if (_delegate && [_delegate respondsToSelector:@selector(investorCellDeleteBtnClick:)]) {
        [_delegate investorCellDeleteBtnClick:self ];
    }
}
